...
## 解法
* 解法1：此题与三数之和解法类似。题目要求找到与目标值 \textit{target}target 最接近的三元组，这里的「最接近」即为差值的绝对值最小。我们枚举的三元组 (a, b, c)满足 $a≤b≤c$，保证了只有 (a, b, c)这个顺序会被枚举到，而 (b, a, c)、(c, b, a)等等这些不会，这样就减少了重复。要实现这一点，我们可以将数组中的元素从小到大进行排序。对于有序的数组，我们在固定a的情况下使用双指针。
